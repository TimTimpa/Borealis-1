# ds3231.py  - minimal DS3231 driver for MicroPython on Raspberry Pi Pico
...
class DS3231:
    ...
    def datetime(self, dt=None):
        """
        If dt is None: read and return datetime tuple:
            (year, month, day, weekday, hour, minute, second, subseconds)

        If dt is given: set RTC time from that tuple.
        Weekday is 1=Monday ... 7=Sunday (you can just pass 1 if you don't care).
        """
        if dt is None:
            ...
            hour = self._bcd2dec(data[2] & 0x3F)   # 24h mode
        ...
        year -= 2000  # store as 00–99
        buf = bytearray(7)
        buf[0] = self._dec2bcd(second)
        buf[1] = self._dec2bcd(minute)
        buf[2] = self._dec2bcd(hour)   # 24h mode
